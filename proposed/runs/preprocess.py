...
def build_uniform_noise(num_class: int, noise_prob: float, noise_type: str) -> np.ndarray:
    ...
    if noise_type.__eq__('sym'):
        ...
        print("noise matrix")
        print(noise_matrix)
    elif noise_type.__eq__("asym"):
        # pair
        temp_matrix = np.zeros(mat_size)
        for i in range(mat_size[0]):
            temp_matrix[i][ i + 1 - mat_size[0]] = noise_prob
        noise_matrix = (1 - noise_prob) * np.identity(num_class) + temp_matrix
        # (noise_prob / (num_class - 1)) * (np.ones(mat_size) - np.eye(*mat_size))
        print("noise matrix")
        print(noise_matrix)
