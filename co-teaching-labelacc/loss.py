...
# Loss functions
def loss_coteaching(y_1, y_2, t, forget_rate, ind, noise_or_not, h1, h2, clean_labels):
    ...
    # 计算label的相关指标
    print(y_1_refurb[0])
    # print(np.argsort(-y_1_refurb.data.cpu(),axis=1)[0])
    y_1_refurb = [i[0] for i in np.argsort(-y_1_refurb.data.cpu(),axis=1)]
    print(y_1_refurb[0])
    y_1_clean_labels = clean_labels[ind[ind_1_sorted[(num_remember):]]]
    y_1_noise_or_not = noise_or_not[ind[ind_1_sorted[(num_remember):]]]
    # 修改正确的样本
    correct = (y_1_refurb==y_1_clean_labels)
    TP = []
    FP = []
    FN = []
    TN = []
    for i in range(len(y_1_refurb)):
        if y_1_noise_or_not[i] == 1:
            TP.append(correct[i])
            FN.append(1-correct[i])
        else:
            FP.append(correct[i])
            TN.append(1-correct[i])
    
    
           
    return loss_1_update,loss_2_update, pure_ratio_1, pure_ratio_2, [entropy_1, entropy_2], \
            [int(np.sum(TP)),int(np.sum(FN)),int(np.sum(FP)),int(np.sum(TN))]
