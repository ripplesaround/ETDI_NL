...
def train(train_loader,epoch, model1, optimizer1, model2, optimizer2, num_classes):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        loss_1, loss_2, pure_ratio_1, pure_ratio_2, entropy_list, acc_list = loss_coteaching(logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, re_1,re_2, clean_labels)
        ...
        TP, FP, FN, TN  = acc_list
        correct_rate = (FP+1) / (FP+TN+1)
        ...
        if (i+1) % args.print_freq == 0:
            print ('Epoch [%d/%d], Iter [%d/%d] Training Accuracy1: %.4F, Training Accuracy2: %.4f, Loss1: %.4f, Loss2: %.4f, Pure Ratio1: %.4f, Pure Ratio2 %.4f, correct_rate: %.4f' 
                  ...
                  np.sum(correct_rate_list)/len(correct_rate_list)))

    train_acc1=float(train_correct)/float(train_total)
    train_acc2=float(train_correct2)/float(train_total2)
    # print(y_star[0:10], cnt_total)
    # np.mean(np.array(entropy_1))
    return train_acc1, train_acc2, pure_ratio_1_list, pure_ratio_2_list, [np.mean(np.array(entropy_1)),np.mean(np.array(entropy_2))], correct_rate_list
...
def main():
    ...
    for epoch in range(1, args.n_epoch):
        ...
        train_acc1, train_acc2, pure_ratio_1_list, pure_ratio_2_list , entropy_list, correct_rate_list =train(train_loader, epoch, cnn1, optimizer1, cnn2, optimizer2, num_classes)
        entropy_1.append(entropy_list[0])
        entropy_2.append(entropy_list[-1])
        # evaluate models
        test_acc1, test_acc2=evaluate(test_loader, cnn1, cnn2)
        ...
        correct_rate = sum(correct_rate_list) / len(correct_rate_list)
        print('Epoch [%d/%d] Test Accuracy on the %s test images: Model1 %.4f %% Model2 %.4f %%, Pure Ratio 1 %.4f %%, Pure Ratio 2 %.4f %%, correct_rate %.4f %%' % (epoch+1, args.n_epoch, len(test_dataset), test_acc1, test_acc2, mean_pure_ratio1, mean_pure_ratio2, correct_rate))
        with open(txtfile, "a") as myfile:
            myfile.write(str(int(epoch)) + ': '  + str(train_acc1) +' '  + str(train_acc2) +' '  + str(test_acc1) + " " + str(test_acc2) + ' ' + str(mean_pure_ratio1) + ' ' + str(mean_pure_ratio2) + "\n")

    # plot
    xrange = [(i + 1) for i in range(args.n_epoch-1)]
    plt.plot(xrange, entropy_1, 'b', label='entropy_1')
    plt.plot(xrange, entropy_2, 'r', label='entropy_2')
    plt.legend()
    plt.title('entropy_1 obtained from the class distribution')
    plt.savefig('./pic/{}_{}.png'.format("cifar10",args.n_epoch))
    plt.clf()
