...
def main():
    ...
    train_acc_plot.append(0.0)
    entropy_1.append(test_acc1)
    entropy_2.append(test_acc2)
    ...
    for epoch in range(1, args.n_epoch):
        ...
        train_acc_plot.append(train_acc2)
    ...
    # plot
    xrange = [(i + 1) for i in range(len(entropy_1))]
    plt.plot(xrange, entropy_1, 'b', label='clean')
    plt.plot(xrange, entropy_2, 'r', label='noisy')
    plt.plot(xrange, train_acc_plot, 'g', label='train acc')
    plt.legend()
    # plt.title('entropy_1 obtained from the class distribution')
    plt.ylabel('Accuracy', fontdict={'family' : 'Times New Roman', 'size'   : 18})
    plt.xlabel('Epochs', fontdict={'family' : 'Times New Roman', 'size'   : 18})
    plt.savefig('./pic/{}_{}_use1.svg'.format("cifar10",args.n_epoch),format="svg",dpi=160)
    plt.clf()
