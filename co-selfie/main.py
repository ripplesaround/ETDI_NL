...
# Train the Model
def train(train_loader,epoch, model1, optimizer1, model2, optimizer2, history_q_1, history_q_2):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        loss_1, loss_2, pure_ratio_1, pure_ratio_2, history_q_1,len_refurb_1, history_q_2,len_refurb_2 = loss_coteaching(logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, re_1,re_2, history_q_1,history_q_2, epoch)
        # entropy_1.append(entropy_list[0])
        # entropy_2.append(entropy_list[1])
        # cnt_total += cnt
        pure_ratio_1_list.append(100*pure_ratio_1)
        pure_ratio_2_list.append(100*pure_ratio_2)
        len_refurb_list_1.append(len_refurb_1)
        ...
        if (i+1) % args.print_freq == 0:
            print ('Epoch [%d/%d], Iter [%d/%d] Training Accuracy1: %.4F, Training Accuracy2: %.4f, Loss1: %.4f, Loss2: %.4f, Pure Ratio1: %.4f, Pure Ratio2 %.4f, len_refurb %.4f' 
                  ...
                  np.sum(len_refurb_list_1)))

    train_acc1=float(train_correct)/float(train_total)
    train_acc2=float(train_correct2)/float(train_total2)
    # print(y_star[0:10], cnt_total)
    # np.mean(np.array(entropy_1))
    return train_acc1, train_acc2, pure_ratio_1_list, pure_ratio_2_list, history_q_1, history_q_2
...
def main():
    ...
    # 历史消息矩阵
    q = 10
    history_q_1 = np.zeros([len(train_dataset),q], dtype = int) # 50000 * 15
    history_q_2 = np.zeros([len(train_dataset),q], dtype = int) # 50000 * 15
    ...
    for epoch in range(1, args.n_epoch):
        ...
        train_acc1, train_acc2, pure_ratio_1_list, pure_ratio_2_list, history_q_1, history_q_2  = train(train_loader, epoch, cnn1, optimizer1, cnn2, optimizer2, history_q_1, history_q_2)
        # entropy_1.append(entropy_list[0])
        # entropy_2.append(entropy_list[-1])
        # evaluate models
        test_acc1, test_acc2=evaluate(test_loader, cnn1, cnn2)
