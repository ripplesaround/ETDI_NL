...
if args.dataset=='cifar10':
    ...
    test_dataset_clean = CIFAR10(root='../data/',
                                download=True,  
                                train=False, 
                                transform=transforms.ToTensor(),
                                noise_type="clean",
    ...
    test_dataset_noisy = CIFAR10(root='../data/',
                                download=True,  
                                train=False, 
                                transform=transforms.ToTensor(),
                                noise_type=args.noise_type,
...
def train(train_loader,epoch, model1, optimizer1, model2, optimizer2, num_classes):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        loss_1, loss_2 = loss_coteaching(logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, re_1,re_2)
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset,
                                              ...
                                              drop_last=True,
                                              shuffle=False)
    test_loader_clean = torch.utils.data.DataLoader(dataset=test_dataset_clean,
                                              batch_size=batch_size, 
                                              num_workers=args.num_workers,
                                              drop_last=True,
                                              shuffle=False)
    test_loader_noisy = torch.utils.data.DataLoader(dataset=test_dataset_noisy,
                                              batch_size=batch_size, 
                                              num_workers=args.num_workers,
                                              drop_last=True,
                                              shuffle=False)
    ...
    test_acc1_list = []
    test_acc2_list = []
    test_clean_acc1_list = []
    test_clean_acc2_list = []
    test_noisy_acc1_list = []
    test_noisy_acc2_list = []
    ...
    test_acc1, test_acc2=evaluate(test_loader_clean, cnn1, cnn2)
    test_clean_acc1_list.append(test_acc1)
    test_clean_acc2_list.append(test_acc2)
    test_acc1, test_acc2=evaluate(test_loader_noisy, cnn1, cnn2)
    test_noisy_acc1_list.append(test_acc1)
    test_noisy_acc2_list.append(test_acc2)
    ...
    for epoch in range(1, args.n_epoch):
        ...
        train_acc1, train_acc2  =train(train_loader, epoch, cnn1, optimizer1, cnn2, optimizer2, num_classes)
        train_acc1_list.append(train_acc1)
        train_acc2_list.append(train_acc2)
        # evaluate models
        test_acc1, test_acc2=evaluate(test_loader, cnn1, cnn2)
    ...
    # plot
    xrange = [(i + 1) for i in range(args.n_epoch)]
    plt.plot(xrange, train_acc1_list, 'r', label='train')
    plt.plot(xrange, test_acc1_list, 'skyblue',linestyle='dashed', label='nr=0.5')
    plt.plot(xrange, test_clean_acc1_list, 'lightgreen',linestyle='dashed', label='nr=0')
    plt.plot(xrange, test_noisy_acc1_list, 'orange',linestyle='dashed', label='nr=1')
    plt.legend()
    plt.title('model1 noise')
    plt.savefig('./pic/{}_{}_{}.png'.format("only clean1",args.n_epoch,'noise'))
    plt.clf()

    xrange = [(i + 1) for i in range(args.n_epoch)]
    plt.plot(xrange, train_acc2_list, 'r', label='train')
    plt.plot(xrange, test_acc2_list, 'skyblue',linestyle='dashed', label='nr=0.5')
    ...
    plt.legend()
    plt.title('model2 noise')
    plt.savefig('./pic/{}_{}_{}.png'.format("only clean2",args.n_epoch,'noise'))
    plt.clf()
