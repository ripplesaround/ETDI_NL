...
def loss_coteaching(y_1, y_2, t, forget_rate, ind, noise_or_not, h1, h2):
    ...
    # only get clean
    y_clean = noise_or_not[ind]
    print("here")
    y_1 = y_1[y_clean.nonzero()]
    y_2 = y_2[y_clean.nonzero()]
    t = t[y_clean.nonzero()]
    print(y_clean)
    print(y_1.size())
    print(t.size())
    ...
    # loss_1_update = F.mse_loss(y_1, t_one_hot)
    # loss_2_update = F.mse_loss(y_2, t_one_hot)
    # smooth_l1_loss
    # t_one_hot = label_trans(t, y_1.size()[-1])
    # loss_1_update = F.smooth_l1_loss(y_1, t_one_hot)
    # loss_2_update = F.smooth_l1_loss(y_2, t_one_hot)
    # nll_loss
    # loss_1_update = F.nll_loss(y_1, t)
    # loss_2_update = F.nll_loss(y_2, t)
